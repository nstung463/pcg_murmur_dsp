...
deep_dive_cells = [
    ...
    | **Nyquist check** | `f_s >= 2 f_max` (hay `\Omega_s >= 2\Omega_N`) | `target_fs = 1000` Hz, band quan tâm đến 400 Hz nên `1000 >= 2·400` | **Lecture 2, Slide 7** |
    ...
    | **Band-pass filter** | `Y(e^{j\omega}) = H(e^{j\omega})X(e^{j\omega})`; lý tưởng: `H_bp(e^{j\omega}) = 1` trong `[\omega_1,\omega_2]`, 0 ngoài dải | `design_filter(..., "butterworth", 25, 400, ...)` + `apply_filter()` | **Lecture 4**, *Frequency Analysis of Signals & Systems*, **Slide 4 và 7** |
