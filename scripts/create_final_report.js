...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      para('4.3. CNN baseline và ablation — 7'),
      ...
      heading('4.3. CNN baseline và ablation', HeadingLevel.HEADING_2),
      para('Để đối chiếu với hướng spectrogram CNN, một MobileNetV3-Small pretrained được thêm như baseline deep-learning nhẹ. CNN dùng input 1 kHz, 16-bit, không filter; mỗi recording chia tối đa hai cửa sổ 3 giây, chuyển thành log-STFT 128×128 và đưa qua backbone frozen; xác suất các cửa sổ được gộp về patient-level. Đây là full labeled cohort nhưng không phải full fine-tuning và không dùng mọi cửa sổ, nhằm giữ thời gian CPU thực tế.'),
      table(['CNN setting', 'Accuracy', 'Balanced acc.', 'Macro-F1'], [['Mean + tuned threshold', '0,841', '0,666', '0,697'], ['Median + tuned threshold', '0,833', '0,689', '0,710'], ['Top-25% windows', '0,765', '0,660', '0,653'], ['Max window', '0,667', '0,543', '0,536'], ['Median + last block fine-tuned', '0,841', '0,680', '0,708']], [4400, 1500, 1800, 1600]),
      caption('Bảng 6. CNN aggregation, threshold và fine-tuning ablation; test = 132 bệnh nhân.'),
      para('Median aggregation là lựa chọn CNN tốt nhất trong seed 42. Max và top-25% dễ bị chi phối bởi một window có xác suất cao bất thường. Fine-tune block cuối không cải thiện Macro-F1 so với frozen backbone. Khi cố định patient split seed 42 và đổi model seed 42/7/123, Macro-F1 trung bình là 0,688 ± 0,025; vì test chỉ có 27 Present patients nên cần báo cáo độ dao động thay vì chỉ một con số.'),
      ...
      bullet('Benchmark full-cohort có test set 132 bệnh nhân; CNN ablation đã kiểm tra thêm nhiều model seed trên cùng split, nhưng matrix SVM/MLP chính vẫn dùng seed 42.'),
      ...
      bullet('CNN là baseline transfer-learning nhẹ với backbone frozen; chưa có kết quả full fine-tuning toàn backbone hoặc calibration xác suất.'),
